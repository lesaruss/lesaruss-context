...
### 1.9 Mistake postmortem before fix (LOCKED 2026-05-05)

The moment Sean issues a definitive correction (no, wrong, stop, you missed, you forgot, I told you before, this keeps happening, again) or Claude self-detects a rule violation in its own output, a postmortem diagnostic runs BEFORE the fix is applied. The diagnostic is two questions. Question one: NEW variable or REPEAT of a known pattern? Search `console/locked-rules.md` and local auto-memory for prior art. Question two (only if REPEAT): what allowed this through, was the rule loaded, was the rule clear, was the rule in the right place, did a related skill fail to fire? On REPEAT, fix the rule infrastructure FIRST, then the mistake. On NEW, fix the mistake; save a memory if the variable will plausibly recur. Day 1 visibility is one line on REPEAT, silent on NEW. Sunset to silent on REPEAT after 30 days if the false-positive rate is healthy and Sean has stopped checking the postmortem log.

This is the BEHAVIORAL counterpart to rules 1.7 and 1.8. Rule 1.7 prevents off-brand artifacts at share time. Rule 1.8 prevents stale-rule sessions at start time. Rule 1.9 catches drift that escapes both gates and feeds it back into the rules so the same shape of mistake does not recur. The three rules form a prevent-verify-repair cycle.

**Why:** the 2026-05-05 conversation. Sean's own theory of "how can we work where you don't make mistakes" was that variables outside our control should be pulled inside our control, then iterated. Diagnosis: without an explicit postmortem step, the fix-the-mistake instinct beats the fix-the-system instinct, mistakes get patched but the rule infrastructure stays the same, the same shape of error recurs, and the "why does this keep happening" conversation runs again. Prior-art search by `vetted-pattern-first`: ADAPT postmortem culture from Google SRE blameless postmortems, the Five Whys (Toyota Production System), DevOps incident retrospectives, and Gary Klein's pre-mortem and postmortem framing. Pattern is mature, well-vetted, and has clear analogs in agentic AI safety (post-action reflection, OODA loops). Trigger surface (the moment a correction is issued) and the two-question diagnostic are LESARUSS-specific.

**How to apply:** the `lesaruss-mistake-postmortem` skill enforces this rule. It fires automatically on the trigger phrases above and on Claude's own self-detection of a rule violation. Never hand-roll the diagnostic. The skill is the single source of truth and inherits across every station via the standard skill-load chain. Every REPEAT detection appends one line to `console/postmortem-log.md` for empirical calibration data. The visibility sunset clause is part of the rule, not a manual review: the skill itself checks the log on session start and may auto-quiet if conditions are met for two consecutive weeks. Composes with rule 1.7 (per-link gate), rule 1.8 (per-session gate), and rule 5.3 (Memory Routing Protocol) which determines whether a NEW variable's saved memory goes to the repo or to local auto-memory. Meta-failures (rule 1.7 or 1.8 itself failing to fire) are logged with the prefix `META:` and surfaced to Sean immediately, never auto-repaired.
...
- 2026-05-05, v2.1. Added rule 1.9 (Mistake postmortem before fix) plus skill `lesaruss-mistake-postmortem` and `console/postmortem-log.md`. Triggered by the 2026-05-05 conversation, Move Three. Sean asked which version would best serve the organization; recommended visible-on-REPEAT with a 30-day sunset clause. Pattern is postmortem culture (Google SRE blameless postmortems, Five Whys, DevOps retrospectives, Klein pre/post-mortems), prior-art-checked. Behavioral counterpart to rules 1.7 and 1.8: prevent (1.7, 1.8) + repair (1.9) = full loop. Day 1 visible on REPEAT, silent on NEW. Sunset to silent on REPEAT after 30 days if calibration is healthy.
